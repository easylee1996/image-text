...
async def goto_download_cover():
    ...
    try:
        ...
        width, height = 1920, 1080
        async with async_playwright() as p:
            # browser = await p.chromium.launch(headless=False, downloads_path=str(download_dir))
            browser = await p.chromium.launch(headless=True, downloads_path=str(download_dir))
            context = await browser.new_context(
                permissions=["clipboard-read"],
                viewport={"width": width, "height": height}
            )
            ...
            await page.goto(
                f"http://127.0.0.1:6173/#/editor"
            )

            await page.wait_for_selector("//div[@class='contentBox']")

            # 监听下载事件
            async with page.expect_download() as download_info:
                await page.click("//button[text()=' 直接下载 ']")

            download = await download_info.value
            download_path = await download.path()

            # 设置期望的文件名
            expected_filename = "cover.webp"  # 替换为您期望的文件名
            target_path = download_dir / expected_filename

            # 重命名下载的文件
            download_path.rename(target_path)
            print(f"Renamed file to: {target_path}")
