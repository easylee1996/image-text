  ...
  <div class="wrap">
    <div class="other-text-wrap">
          ...
          <div v-for="(cate, index) in props.cateList" :key="index + 't'">
            ...
            <div class="list-wrap" v-if="cate.list.length > 0">
              <div v-for="(item, i) in cate.list" :key="i + 'sl'" draggable="false" @click="handleClick(item)">
                <a-image v-if="i < 3" class="list__img-thumb" height="95" width="95" :preview="false" fit="contain"
                  :src="item.url" />
              </div>
            </div>
          </div>
      ...
      <ul v-if="currentCate" class="list">
        <a-list :gridProps="{ gutter: [5, 10], span: 8 }" :bordered="false" :data="props.data" @reach-bottom="fetchData"
          ...
          <template #item="{ item, index }">
            <a-list-item style="padding: 0" @click="handleClick(item)" :key="index">
              <div class="list__img-thumb">
                <a-image class="list__img" :preview="false" :height="95" width="100%" fit="contain" :src="item.url" />
              </div>
            </a-list-item>
          </template>
