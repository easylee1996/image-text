...
setupMock({
    setup() {
        ...
        // 修改为随机获取一个记录
        // Mock.mock(new RegExp('/api/template/templateList'), (params) => {
        //     const randomIndex = Math.floor(Math.random() * templateData.list.length);
        //     const randomRecord = templateData.list[randomIndex];
